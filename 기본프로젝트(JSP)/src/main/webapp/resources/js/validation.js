var validate = (function() {
  ...
  var validateData = {
	...
	validateObj.addValidate = function (target,data){
		...
		target.onchange = function(event){
			if(this.validateData.regType != "" && !checkValidate(this,event)){
				innerMsg(this," 형식에 맞지 않습니다.");
				this.validateData.availability = false;
				event.stopImmediatePropagation();
				return false;
			}

      if(this.validateData.regType == "ID"){
      const _this = this; // this를 콜백에서 쓰기 위해 저장

      checkUser(this.value, function(isAvailable){
        if(isAvailable){
          innerMsg(_this, "사용 가능한 ", "test");
          _this.validateData.enable = true;
        } else {
          innerMsg(_this, "사용 불가한 ", "test");
          _this.validateData.enable = false;
          event.stopImmediatePropagation();
          return false;
        }
      });
	...
	validateObj.checkValidate = function (event){
		...
		for(var i in list){
			...
			if(param.validateData.required == "Y" && param.value == ""){
				gfnAlert(param.title + "는 필수 입력사항입니다.");
				return false;
			}
			if(param.validateData.regType != "" && !checkValidate(param,"change")){
				gfnAlert(param.title + " 형식에 맞지 않습니다.");
				return false;
			}
      if(param.validateData.regType == "ID" && param.validateData.enable === false ){
				gfnAlert("중복된 " + param.title + " 입니다.");
				return false;
			}
		}
		return true;
	...
	function innerMsg(target,msg,type){
		var textNode = document.createElement('p');
		if(target.validateData.msgType == "TEXT"){
			if(target.nextElementSibling != null && target.nextElementSibling.nodeName == "P"){
        console.log("여긴 오나111?");
				target.nextElementSibling.remove();
			}
      if (type == "test"){
        console.log("여긴 오나111?");
        textNode.innerText = msg+target.validateData.title+"입니다.";
      } else {
        console.log("여긴 오나222?");
        textNode.innerText = target.validateData.title+msg;
      }
      
      console.log("textNode.innerText -> ", textNode.innerText);
			target.insertAdjacentElement('afterend',textNode);
		}else if(target.validateData.msgType == "ALERT"){
			gfnAlert(target.validateData.title+msg);
		}
	}
  ...
  function checkUser(userNm, callback){
    var data = { userNm: userNm };
    ajaxCall("/checkUser", "POST", data, function(res){
      console.log("왔다갔나? ", res.result);
      callback(res.result);
    });
  }
